...
class GsMilkClientTest(unittest.TestCase):
    ...
    def test_create_collection_with_schema_and_partition_expr(self):
        ...
        schema = self.client.create_schema(
            partitions=GsHashPartition("id", part_count=3)
        )
        schema.add_field(field_name="id", datatype=DataType.INT64, is_primary=True)
        schema.add_field(field_name="title", datatype=DataType.VARCHAR, max_length=512)
        schema.add_field(
    ...
    def test_insert_data(self):
        ...
        data = [
            {"id": 12, "embedding": [1, 2, 3], "meta": {"doc": "gaussdb document 1"}},
            {
                "id": 90,
                "embedding": [0.13, 0.123, 1.213],
                "meta": {"doc": "gaussdb document 1"},
            },
        ]
        self.client.insert(
    ...
    def test_delete_data(self):
        ...
        self.client.delete(
            collection_name=test_collection_name, ids=[90, 112], filter=where_clause
        )
    ...
    def test_query_and_get(self):
        ...
        data = [
            {"id": 12, "embedding": [1, 2, 3], "meta": {"doc": "GuassDB document 1"}},
            {
                "id": 90,
                "embedding": [0.13, 0.123, 1.213],
                "meta": {"doc": "GuassDB document 1"},
            },
        ...
        res = self.client.query(
            collection_name=test_collection_name, filter=where_clause
        )
        self.assertEqual([row["id"] for row in res], [12, 90])
        self.assertEqual(
            [row["meta"] for row in res],
            [{"doc": "GuassDB document 1"}, {"doc": "GuassDB document 1"}],
        )
        ...
        self.assertEqual(
            res,
            [
                {"id": 12, "meta": {"doc": "GuassDB document 1"}},
                {"id": 112, "meta": None},
            ],
        )
    ...
    def test_drop_index(self):
        ...
        self.client.drop_index(collection_name=test_collection_name, index_name="vidx9")
