...
class PartitionFieldException(GsException):
    """Raise when partition field invalid"""


class PrimaryKeyException(GsException):
    """Raise when primary key are invalid"""


class VectorFieldParamException(GsException):
    """Raise when Vector Field parameters are invalid"""


class VarcharFieldParamException(GsException):
    """Raise when Varchar Field parameters are invalid"""


class ArrayFieldParamException(GsException):
    """Raise when Array Field parameters are invalid"""


class CollectionStatusException(GsException):
    """Raise when collection status is invalid"""


class VectorMetricTypeException(GsException):
    """Raise when vector metric type is invalid"""


class MilvusCompatibilityException(GsException):
    """Raise when compatibility conflict with milvus"""


class ClusterVersionException(GsException):
